...
for(i in 1:N){
  ...
  y_scale_vec[i] <- sqrt(t(y_temp)%*%W[temp_ind,temp_ind]%*%y_temp)
}

#y_scale <- sqrt(norm(y_common))
y_scale <- mean(y_scale_vec)
...
one_M <- rep(1,M)
one_N <- rep(1,N)
lambda_pivot <- lambda_y_save[,,n_save]*(one_M%*%t(sqrt(psi_y_save[,n_save])))
eta_pivot <- eta_y_save[,,n_save]/(sqrt(psi_y_save[,n_save])%*%t(one_N))
...
for(iter in 1:length(post_burn_iter)){
  lambda_post <- lambda_y_save[,,iter]*(one_M%*%t(sqrt(psi_y_save[,iter])))
  eta_post <- eta_y_save[,,iter]/(sqrt(psi_y_save[,iter])%*%t(one_N))
...
y_scale_norm <- array(0,dim = c(K,n_save))
y_cum <- array(0,dim = c(K,n_save))
for(iter in 1:n_save){
  lambda_temp <- y_scale*lambda_save_processed[,,iter]
  y_scale_norm[,iter] <- sort(sqrt(diag(t(lambda_temp)%*%W%*%lambda_temp)),decreasing = T)
}
norm_df <- melt(y_scale_norm[,(n_save/2):n_save])
#scree plot
