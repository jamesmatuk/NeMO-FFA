...
// [[Rcpp::export]]
arma::mat test_fun(const arma::vec vector,const int n) {
  return arma::repmat(vector,1,n);
}
...
arma::vec l_mu_MH(arma::mat y_st, const arma::vec grid, const arma::mat obs_grid, double l_param_cur, 
  ...
  // double l_cur = evaluate_mu_cov(y_st, cov_mat_cur, sigma_sq_cur, obs_grid);
  // double l_can = evaluate_mu_cov(y_st, cov_mat_can, sigma_sq_cur, obs_grid);
